...
import { conversationIdAtom, conversationsAtom, LoadConvoAtom, recepientIdAtom } from "../store/store";
...
export function ContactCard(
    ...
    return(
        <div className="flex px-3 py-2 items-center overflow-ellipsis relative">
            <button className="grow flex gap-3 items-center" 
                onClick={clickHandler}

            >
                <div className="rounded-full flex items-center">
                    <img className="rounded-full min-w-14 min-h-14" 
                            height={40} width={40} src={`${conversation.participants[0]?.user.profilePicture ? conversation.participants[0]?.user.profilePicture : "/default_Profile.png"}`}
                             alt="Profile-Picture">
                    </img>
                </div>
                <div className="grow flex justify-between">
                    <div className="pl-4">
                        ...
                        <p className="text-left text-sm max-w-56 break-words text-ellipsis line-clamp-1">
                            {
                               conversation.messages[0]?.messageType === "text" ?  (conversation?.messages[0]?.content) :
                               (
                                    "Image is sent"
                               )
                            }
                    ...
                    <div className="flex items-start text-[12px] font-normal text-gray-400">
                        {ModifiedTimeAgo(conversation?.messages[0]?.createdAt || new Date())}
                    </div>
                </div>
            </button>
        </div>
